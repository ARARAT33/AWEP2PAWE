...
const DB='AWEP2PAWE_DB', STORE='app-state', KEY='root', IDENTITY='identity', LEGACY='AWEP2PAWE_STATE_V11', CORE='awep2pawe:v6', VERSION=4;
let dbPromise;
...
async function ensureIdentity(){
  ...
  try{
    ...
    if(existing?.uid===uid&&existing.privateKey instanceof CryptoKey)return existing;
    if(!crypto.subtle)return {uid};
    /* Non-extractable private key: IndexedDB can persist CryptoKey objects without exposing key bytes to JS. */
    const pair=await crypto.subtle.generateKey({name:'ECDSA',namedCurve:'P-256'},false,['sign','verify']);
    const publicKey=await crypto.subtle.exportKey('jwk',pair.publicKey);
    const identity={id:'primary',uid,algorithm:'ECDSA-P256-SHA256',publicKey,privateKey:pair.privateKey,createdAt:Date.now(),version:2};
